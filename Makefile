BUILD = build
MAKEFILE = Makefile
OUTPUT_FILENAME = "Auf_der_sibirischen_Bahn_nach_China"
TITLE_NAME = "Auf der sibirischen Bahn nach China"
...
$(BUILD)/pdf/$(OUTPUT_FILENAME).pdf: $(MAKEFILE) $(METADATA) $(CHAPTERS) $(CSS_FILE) $(IMAGES) $(COVER_IMAGE) $(METADATA_PDF) $(PREFACE_EPUB)
	mkdir -p $(BUILD)/pdf
	cp  *.css  $(IMAGES_FOLDER)
	cp  $(IMAGES_FOLDER)/Auf_der_sibirischen_Bahn_nach_China_*.jpg .
	...
	rm  $(IMAGES_FOLDER)/*.css
	rm Auf_der_sibirischen_Bahn_nach_China_*.jpg
	rm cover1.jpg 
	rm logo.jpg
